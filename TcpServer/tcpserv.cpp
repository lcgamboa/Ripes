...
int main(int argc, char* argv[]) /* simple TCP server */
{
        ...
        while (!exit_loop) {
            ...
            switch (cmd_header.msg_type) {
                ...
                case VB_PREAD: {
                    ...
                    if (addr > 8) {
                        printf("invalid reg addr %i !!!!!!!!!!!!!!!!!!\n", addr);
                        addr = 0;
                    }
                    uint32_t payload[2];
                    payload[0] = htonl(addr);
                    payload[1] = htonl(regs[addr]);
...
int32_t send_cmd(const uint32_t cmd, const char* payload, const uint32_t payload_size) {
    uint32_t ret;
    cmd_header_t cmd_header;
